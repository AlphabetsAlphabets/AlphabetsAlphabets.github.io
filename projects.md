---
layout: page
title: projects
---
Here you can see the number of projects I have active.
1. [rlox](https://yjh16120.github.io/rlox), a clone of the lox programming language but written in Rust hence the 'r' prefix before 'lox'.
2. [Kalculator](https://github.com/YJH16120/Kalculator). It's a calculator. The same knowledge needed to create a programming language is also needed in making a calculator, I thought why not kill two birds with one shot.

# Ideas
Here you'll look at my project ideas, basically what I want to create in the future.
1. Creating a text editor with basic features.
2. Creating a music discord bot. I currently have a discord bot that can play music, but it's very ineffective. And I might need to refactor the entire song playback feature.
3. A shitpost generator. "Because I must shitpost at unprecedented speeds" - nth. Someone on my [discord](https://discord.gg/DsmRhYYUH9).
4. Creating a discord bot, that can generate custom text channels for a specific person with a specific topic. The user can then discard the channel. The channel will then be deleted.
